...
export const Budget: React.FC<BudgetProps> = ({ 
  ...
  // Sync state if props change (e.g. cloud load)
  useEffect(() => {
  ...
  return (
    <div className="pb-32 animate-in fade-in duration-500 space-y-6">
      <header className="px-6 pt-safe-top pb-2 flex items-center justify-between">
        ...
        <button 
            onClick={handleSave}
            className={`p-3 rounded-xl transition-all duration-300 shadow-md flex items-center gap-2 font-bold ${
                isSaved 
            ...
            <div className="grid gap-5">
                ...
                <div className="relative">
                     ...
                     <div className="relative">
                        ...
                        <input
                            type="number"
                            value={localOverall.monthly || ''}
                            onChange={e => setLocalOverall({...localOverall, monthly: parseFloat(e.target.value) || 0})}
                            className="w-full pl-8 pr-4 py-3 bg-white/60 dark:bg-slate-900/60 border border-white/40 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-purple-500/50 outline-none transition-all text-slate-900 dark:text-white font-bold"
                            placeholder="0.00"
                        />
                     </div>
                </div>
